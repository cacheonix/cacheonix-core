      ...
      if (servletRequest instanceof HttpServletRequest) {
         ...
         if (entry == null) {
            ...
            if (createdTime != null) {
               ...
               final long longIfModifiedSince = httpServletRequest.getDateHeader(HEADER_IF_MODIFIED_SINCE);
            ...
            final Map<String, Collection<Header>> cachedHeaders = cachedResponseValue.getHeaders();
            for (final Entry<String, Collection<Header>> headerEntry : cachedHeaders.entrySet()) {
            ...
            for (final Entry<String, Header> clientCacheHeaderEntry : clientCacheHeaders.entrySet()) {
               ...
               if (cachedHeaders.containsKey(clientCacheHeaderName)) {
                  continue;
               }
            ...
            if (binaryResponse != null) {
               ...
               if (textContentType && !compressed) {

                  // Doesnt current request accept gzip encoding?
                  final String acceptEncoding = httpServletRequest.getHeader(HEADER_ACCEPT_ENCODING);
                  final String[] encodings = acceptEncoding.split(",");
                  for (int i = 0; i < encodings.length && !outputGzip; i++) {
                     if (GZIP.equalsIgnoreCase(encodings[i].trim())) {
               ...
               if (outputGzip) {
                  ...
                  // Notify the client that the response is gzip-encoded
                  httpServletResponse.setHeader(HEADER_ENCODING, GZIP);

                  // Gzip cached output
                  final GZIPOutputStream gzipOutputStream = new GZIPOutputStream(outputStream);
                  gzipOutputStream.write(binaryResponse);
                  gzipOutputStream.finish();
               } else {

                  // Write cached output as it is
