 ...
 * <p/>
 * <p>The DTD is specified in <a href="log4j.dtd"><b>log4j.dtd</b></a>.
 * <p/>
 * <p>Sometimes it is useful to see how log4j is reading configuration files. You can enable log4j internal logging by
 ...
 * @since 0.8.3
 */
public final class DOMConfigurator implements Configurator {
   ...
   protected final Appender parseAppender(final Element appenderElement) {
      ...
      try {
         final Object instance = Loader.loadClass(className).getConstructor().newInstance();
               ...
               // Add filters
                  ...
                  if (appender instanceof AppenderAttachable) {
                     ...
                     LogLog.error("Requesting attachment of appender named [" +
                             ...
                             "] which does not implement org.cacheonix.impl.util.logging.spi.AppenderAttachable.");
                  }
               } else {
...
problem: we can't create an Appender */ catch (final Exception oops) {
         ...
         for (int loop = 0; loop < length; loop++) {
            ...
            if (currentNode.getNodeType() == Node.ELEMENT_NODE) {
               ...
               if (tagName.equals(PARAM_TAG)) {
                  ...
                  final String loggerName = currentElement.getAttribute(REF_ATTR);
                  final Logger logger = catFactory == null ? repository.getLogger(loggerName)
   ...
   protected final void parseCategory(final Element loggerElement) {
      ...
      final String className = subst(loggerElement.getAttribute(CLASS_ATTR));
      ...
      synchronized (cat) {
         final boolean additivity = OptionConverter.toBoolean(
   ...
   protected final void parseCategoryFactory(final Element factoryElement) {
      ...
      if (EMPTY_STR.equals(className)) {
         ...
         final Object factory = OptionConverter.instantiateByClassName(className,
                 LoggerFactory.class,
                 null);
         if (factory instanceof LoggerFactory) {
            catFactory = (LoggerFactory) factory;
         } else {
            LogLog.error("Category Factory class " + className + " does not implement org.cacheonix.impl.util.logging.LoggerFactory");
         }
      ...
      for (int loop = 0; loop < length; loop++) {
         ...
         if (currentNode.getNodeType() == Node.ELEMENT_NODE) {
            ...
            if (tagName.equals(APPENDER_REF_TAG)) {
               ...
               final Appender appender = findAppenderByReference(appenderRef);
               final String refName = subst(appenderRef.getAttribute(REF_ATTR));
               if (appender != null) {
   ...
   public final void doConfigure(final String filename, final LoggerRepository repository) {
   ...
   public final void doConfigure(final URL url, final LoggerRepository repository) {
   ...
   private void doConfigure(final ParseAction action, final LoggerRepository repository)
           throws FactoryConfigurationError {
    ...
    */
   public final void doConfigure(final Element element, final LoggerRepository repository) {
