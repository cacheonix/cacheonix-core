 ...
 * certain range.
 * <p/>
 * <p>The filter admits three options <b>LevelMin</b>, <b>LevelMax</b> and <b>AcceptOnMatch</b>.
 * <p/>
 * <p>If the level of the {@link LoggingEvent} is not between Min and Max (inclusive), then {@link Filter#DENY} is
 * returned.
 * <p/>
 * <p> If the Logging event level is within the specified range, then if <b>AcceptOnMatch</b> is true, {@link
 * Filter#ACCEPT} is returned, and if <b>AcceptOnMatch</b> is false, {@link Filter#NEUTRAL} is returned.
 * <p/>
 * <p>If <code>LevelMin</code>w is not defined, then there is no minimum acceptable level (ie a level is never rejected
 * for being too "low"/unimportant).  If <code>LevelMax</code> is not defined, then there is no maximum acceptable level
 * (ie a level is never rejected for beeing too "high"/important).
 * <p/>
 * <p>Refer to the {@link AppenderSkeleton#setThreshold setThreshold} method available to <code>all</code> appenders
 * extending {@link AppenderSkeleton} for a more convenient way to filter out events by level.
 *
 * @author Simon Kitching
 * @author based on code by Ceki G&uuml;lc&uuml;
 */
public final class LevelRangeFilter extends Filter {
