...
import org.cacheonix.impl.util.logging.AsyncAppender;
import org.cacheonix.impl.util.logging.spi.LoggingEvent;

/**
 * <code>BoundedFIFO</code> serves as the bounded first-in-first-out buffer heavily used by the {@link AsyncAppender}.
 *
 * @author Ceki G&uuml;lc&uuml;
 * @since version 0.9.1
 */
@SuppressWarnings({"CallToNativeMethodWhileLocked", "FieldAccessedSynchronizedAndUnsynchronized"})
public final class BoundedFIFO {
   ...
   /**
    * Place a {@link LoggingEvent} in the buffer. If the buffer is full then the event is <b>silently dropped</b>. It is
    * the caller's responsibility to make sure that the buffer has free space.
    */
   ...
   final int min(final int a, final int b) {
      return a < b ? a : b;
   }
