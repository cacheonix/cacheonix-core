...
/**
 * The AsyncAppender lets users log events asynchronously. <p/> <p/> The AsyncAppender will collect the events sent to
 * it and then dispatch them to all the appenders that are attached to it. You can attach multiple appenders to an
 * AsyncAppender. </p> <p/> <p/> The AsyncAppender uses a separate thread to serve the events in its buffer. </p> <p/>
 * <b>Important note:</b> The <code>AsyncAppender</code> can only be script configured using the {@link
 * DOMConfigurator}. </p>
 *
...
public final class AsyncAppender extends AppenderSkeleton
    ...
    */
   private final Map discardMap = new HashMap(5);
    ...
    * Nested appenders.
    */
   final AppenderAttachableImpl aai;
   ...
   public void append(final LoggingEvent event) {
      ...
      synchronized (buffer) {
         while (true) {
            final int previousSize = buffer.size();
            ...
            if (blocking
                    && !Thread.interrupted()
                    && !currentThread.equals(dispatcher)) {
               try {
                  buffer.wait();
                  discard = false;
            ...
            //
            //   if blocking is <code>false</code> or thread has been interrupted
   ...
   public void close() {
      ...
      //
      synchronized (appenders) {
         final Enumeration iter = appenders.getAllAppenders();
    ...
    * there will be no effort to extract the location information related to the event. As a result, the event that will
    * be ultimately logged will likely to contain the wrong location information (if present in the log format). <p/>
    * <p/> Location information extraction is comparatively very slow and should be avoided unless performance is not a
    * concern. </p>
    *
    * @param flag <code>true</code> if location information should be extracted.
    */
   ...
   /**
    * Sets the number of messages allowed in the event buffer before the calling thread is blocked (if blocking is true)
    * or until messages are summarized and discarded.  Changing the size will not affect messages already in the
    * buffer.
    *
    ...
    *
    * @param value <code>true</code> if appender should wait until available space in buffer.
    */
    ...
    *
    * @return <code>true</code> if calling thread will be blocked when buffer is full.
    */
   ...
   private static final class DiscardSummary {
       ...
       */
      DiscardSummary(final LoggingEvent event) {
