...
public final class LockQueueImpl implements LockQueue {
   ...
   public void grantLockRequest(final AcquireLockRequest request) {
      ...
      if (request.isReadLock()) {
         ...
         // Validate
         Assert.assertTrue(writeLockOwner == null || writeLockOwner.cameFromRequester(request),
                 "Cannot grant read lock while write lock is held by some other owner: {0}", writeLockOwner);
         ...
         if (writeLockOwner == null) {
            ...
            // Upgrade?
            Assert.assertTrue(!areReadLocksGranted() || isOnlyReadLockCameFrom(request),
                    "Cannot grant write lock while other read locks are present: {0}", request);
            ...
            Assert.assertTrue(writeLockOwner.cameFromRequester(request), "Write lock is already granted: {0}",
                    writeLockOwner);
            writeLockOwner.incrementEntryCount();
         }
      }
   ...
   public boolean equals(final Object o) {
      ...
      final LockQueueImpl lockQueue = (LockQueueImpl) o;

      if (pendingRequests != null ? !pendingRequests.equals(
              lockQueue.pendingRequests) : lockQueue.pendingRequests != null) {
         return false;
      }
      if (readLockOwners != null ? !readLockOwners.equals(
              lockQueue.readLockOwners) : lockQueue.readLockOwners != null) {
         return false;
      }
      if (writeLockOwner != null ? !writeLockOwner.equals(
              lockQueue.writeLockOwner) : lockQueue.writeLockOwner != null) {
         return false;
      }
