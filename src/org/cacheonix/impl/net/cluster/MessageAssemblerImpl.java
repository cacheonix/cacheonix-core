...
final class MessageAssemblerImpl implements MessageAssembler {
    ...
    */
   private static final Logger LOG = Logger.getLogger(MessageAssemblerImpl.class); // NOPMD
...
//      if (LOG.isDebugEnabled()) LOG.debug("Adding frame: " + frame);
      if (parts.isEmpty()) {
         ...
         if (frame.getPartCount() == 1) {
            ...
            assembledMessages.add(new AssembledMessageImpl(message, frame.getSequenceNumber()));
         } else {
         ...
         if (frame.getPartIndex() == parts.getLast().getPartIndex() + 1) {
            ...
            if (frame.getPartIndex() == frame.getPartCount() - 1) {
               ...
               assembledMessages.add(new AssembledMessageImpl(message, firstFrameNumber));
            }
         } else {
