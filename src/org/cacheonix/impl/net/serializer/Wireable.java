...
public interface Wireable {
   ...
   /**
    */
   int TYPE_PASS_BY_REFERENCE_BINARY = DESTINATION_NONE << 8 | 3;

   int TYPE_PASS_BY_COPY_BINARY = DESTINATION_NONE << 8 | 4;

   int TYPE_COMPRESSED_BINARY = DESTINATION_NONE << 8 | 5;

   int TYPE_BUCKET_OWNER = DESTINATION_NONE << 8 | 6;

   int TYPE_BINARY_ENTRY_MODIFIED_EVENT = DESTINATION_NONE << 8 | 7;

   int TYPE_REPLICATED_STATE = DESTINATION_NONE << 8 | 8;

   int TYPE_BUCKET = DESTINATION_NONE << 8 | 9;

   int TYPE_GROUP_MEMBER = DESTINATION_NONE << 8 | 10;

   int TYPE_BUCKET_OWNERSHIP_ASSIGNMENT = DESTINATION_NONE << 8 | 12;

   int TYPE_MD5_HASH_CODE_CALC = DESTINATION_NONE << 8 | 13;

   int TYPE_CLUSTER_VIEW = DESTINATION_NONE << 8 | 14;

   int TYPE_GROUP = DESTINATION_NONE << 8 | 15;

   int TYPE_NODE_ADDRESS = DESTINATION_NONE << 8 | 16;

   int TYPE_REVIVAL_MARKER = DESTINATION_NONE << 8 | 17;

   int TYPE_BUCKET_TRANSFER = DESTINATION_NONE << 8 | 18;

   int TYPE_LOCK_REGISTRY = DESTINATION_NONE << 8 | 19;

   int TYPE_LOCK_QUEUE = DESTINATION_NONE << 8 | 20;

   int TYPE_LOCK_OWNER = DESTINATION_NONE << 8 | 21;

   int TYPE_BINARY_STORE = DESTINATION_NONE << 8 | 22;

   int TYPE_CACHE_STATISTICS = DESTINATION_NONE << 8 | 23;

   int TYPE_BINARY_STORE_ELEMENT = DESTINATION_NONE << 8 | 24;

   int TYPE_LOCK_QUEUE_KEY = DESTINATION_NONE << 8 | 25;

   int TYPE_ENTRY_MODIFICATION_SUBSCRIPTION = DESTINATION_NONE << 8 | 27;

   int TYPE_TIME = DESTINATION_NONE << 8 | 28;

   int TYPE_CACHEABLE_VALUE = DESTINATION_NONE << 8 | 29;

   int TYPE_CACHEABLE_ENTRY = DESTINATION_NONE << 8 | 30;

   int TYPE_TRANSFER_BUCKET_RESULT = DESTINATION_NONE << 8 | 31;

   int TYPE_JOINING_NODE = DESTINATION_NONE << 8 | 32;

   int TYPE_RECEIVER_ADDRESS = DESTINATION_NONE << 8 | 33;
